...
def processing_data(data_path, height, width, batch_size=4, validation_split=0.1):
    ...
    # :param height: 图片高度
    # :param width: 图片宽度
    # :param batch_size: batch: 每个批次的大小，目前为4
    ...
    train_data = ImageDataGenerator(
            ...
            rescale=1. / 255,  
            # 下面的一系列操作是因为目前的数据集不是很大，这些都是对图片进行一些操作
            # 加入一些噪声，比如旋转、切割等
            # 这样可以让模型训练更加有效
            shear_range=0.1,  
    ...
    train_generator = train_data.flow_from_directory(
            ...
            # batch大小，将完整的数据集分成一个个小小的batch扔进去训练
            batch_size=batch_size,
            # 标签类型，这里表示是多分类问题
            class_mode='categorical',
            # 读取数据后，选择是使用那一部分，这里选择的是训练集部分
            subset='training', 
            seed=0) # 可选参数，打乱数据和进行变换时的随机数种子
    validation_generator = validation_data.flow_from_directory(
            ...
            # 这里选择验证集部分，但实际上这里测试集和训练集已经合二为一了。。
            subset='validation',
            seed=0)
...
def model(train_generator, test_generator, save_model_path='results/dnn.h5',
    ...
    # 初始化model Sequential的意思是初始化一个前馈神经网络结构，
    # 可以不断向其中添加神经网络的层（layer)
    model = Sequential()
    ...
    # 将VGG16模型设置为不进行训练，即始终使用网络的参数，后续为了提高效果可以将VGG16模型的参数也一并进行训练，但对内存和显存的消耗会变大，如果不足可以考虑减小batch_size
    base_model.trainable = False

    # 将VGG16模型加入我们的模型
    model.add(base_model)

    # VGG16卷积神经网络会将输入的384*512*3数据 通过一系列操作（卷积、下采样、最大池化等）
    # 将输入的数据转化成较小规模的数据，但依然是三维的，Flatten层的作用就是将三维的数据展开成1维的数据
    model.add(Flatten())

    # 添加一个全连接层用于训练，全连接层是神经网络最基本的结构，即形成多个线性的关系。
    # activation是激活函数，通过激活函数可以将线性关系转化成可以有更强拟合
    # 能力的非线性函数
    ...
    # 编译模型，为模型设定训练时采用的优化函数、损失函数。这里优化函数是adam，损失函数是类别交叉熵。是一种参数更新策略。
...
def main():
    ...
    # 结果储存路径，根据结果可以修改
    save_model_path = './results/model_15-0.94.hdf5' 
    ...
    # 这里选择加载已经训练好的模型， 不需要训练了，用已经训练好的模型
